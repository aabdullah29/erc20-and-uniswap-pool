...
contract Swap is Ownable{
    ...
    function swapBusdTotoken(uint256 Amount,IERC20 _BUSD,IERC20 Token) external {
     require(Token.balanceOf(address(this))>=Amount,"Contract is ran out of funds! "); 
     require(_BUSD==BUSD,"address not matched!"); 
     ...
     uint256 _fee= calculatefees(Amount);
     uint256 remaining=Amount.sub(_fee);
     BUSD.transfer(owner(),_fee);
     Token.transfer(msg.sender,remaining);
    }

    function swapTokenToBusd(uint256 Amount,IERC20 _BUSD,IERC20 Token) external {
    require(Token.balanceOf(address(this))>=Amount,"Contract is ran out of funds! "); 
     require(_BUSD==BUSD,"address not matched!"); 
     require(Token == EverestCash || Token == VenusCash || Token == MetaCash || Token == RoyalArchieCash,"token address not matched");  
